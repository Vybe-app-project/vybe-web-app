...
test('shared helpers: errMsg reads the envelope object and bootstrapAdmin keeps the token off a 429', () => {
  ...
  // errMsg is the shared reader (lib/apiError.parseApiError), which reads the envelope object.
  has(api, 'return parseApiError(e, fallback).message;');
...
test('the admin error helper is pure and reads only the two exposed headers', () => {
  ...
  // Shared with the client core's parser, which must stay import-free (tests/client-policy.test.mjs).
  assert.doesNotMatch(source, /^\s*import\s/m, 'apiError.ts must not import anything');
