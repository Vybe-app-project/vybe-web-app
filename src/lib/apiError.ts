 ...
 *
 * The admin console's helpers (apiErrorDetails, describeAdminError) sit at the
 * end of this file: same shapes, but the server's sentence is kept verbatim and
 * the code and request id are attached for the on-call engineer.
 */
...
/**
 * The admin console's reading of the same shapes (pages/admin/*). Unlike
 * parseApiError it keeps the server's sentence verbatim (an operator wants
 * the API's words), reads Retry-After as delta-seconds or an HTTP date, and
 * describeAdminError attaches the code and the first eight characters of the
 * request id so a report can be matched to a log line. Reads exactly two
 * headers (X-Request-Id, Retry-After), both in the API's CORS exposedHeaders.
 */
...
/** The axios-shaped surface apiErrorDetails reads; structural so fetch-built errors fit too. */
type LooseFailure = {
...
export function apiErrorDetails(e: unknown, fallback = 'Something went wrong', now = Date.now()): ApiErrorDetails {
  ...
  const ax = e && typeof e === 'object' ? (e as LooseFailure) : null;
